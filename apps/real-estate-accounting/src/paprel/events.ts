...
export function connectPaprelEvents(root: HTMLElement, router: Router, handlers: PaprelEventHandlers): () => void {
  ...
  const accountAction = (event: Event) => {
    const { accountId } = (event as CustomEvent<{ accountId: string }>).detail;
    if (accountId) router.navigate(`/accounts/${accountId}/edit`);
  };
  ...
  const listeners: Array<[string, EventListener]> = [
    ...
    ["account-action", accountAction as EventListener], ["account-saved", accountSaved as EventListener],
